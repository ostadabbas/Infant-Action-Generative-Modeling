...
def generate_actions(beta, model, dataset, epoch, params, folder, num_frames=60,
    ...
    # print("remove smpl")
    model.param2xyz["jointstype"] = "vertices" #"vertices" # "smpl"
    ...
    model.eval()
    samples_output = {'pose':[], 'keypoint':[], 'y':[], 'epoch_idx':[]}
    
    with torch.no_grad():
        if not onlygen:
            # Get xyz for the real ones
            visualization["output_xyz"], _ = model.rot2xyz(visualization["output"],
                                                        visualization["mask"],
            ...
            generation["output_xyz"], _ = model.rot2xyz(generation["output"],
                                                     generation["mask"], vertstrans=vertstrans,
                                                     beta=beta)

            outxyz, _ = model.rot2xyz(reconstruction["output"],
            ...
            generation["output_xyz"], generation["output_pose"] = model.rot2xyz(generation["output"],
                                                     generation["mask"], vertstrans=vertstrans,
                                                     beta=beta)
            output = generation["output_xyz"].reshape(nspa, nats, *generation["output_xyz"].shape[1:]).cpu().numpy()
            poses = generation["output_pose"].reshape(nspa, nats, *generation["output_pose"].shape[1:]).cpu().numpy()
            kpts = generation["output_xyz"].reshape(nspa, nats, *generation["output_xyz"].shape[1:]).cpu().numpy()

            for i in range(nspa):
                for j in range(nats):
                    samples_output['pose'].append(np.transpose(poses[i][j], (2, 0, 1)))
                    samples_output['keypoint'].append(np.transpose(kpts[i][j], (2, 0, 1)))
                    samples_output['y'].append(j)
                    samples_output['epoch_idx'].append(-1)
...
def main():
    ...
    for seed in [1]:  # [0, 1, 2]:
        ...
        if varying_beta:
            ...
            betas = [0]
        for beta in betas:
            output, samples_output = generate_actions(beta, model, dataset, epoch, parameters,
            ...
            syn_filename = os.path.join(folder, "synthetic_samples.npy")
            np.save(syn_filename, samples_output)
            print("Saved at: " + syn_filename)
