...
class CAE(nn.Module):
    ...
    def decay_weight(self, epoch, initial_weight, decay_rate=0.001):
        return max(initial_weight * (1 - decay_rate * epoch), 0.001)  # 0.1 - 0.001
        #return initial_weight * np.exp(-decay_rate * epoch)
    '''
    def get_exponential_decrease_weight(self, epoch, w_init, w_min=0.001, decay_rate=0.001):
        weight = w_min + (w_init - w_min) * np.exp(-decay_rate * epoch) 
        return max(weight, w_min)
    '''
    '''
    def get_exponential_increase_weight(self, epoch, w_init, w_min=1.0, decay_rate=0.001):
        weight = w_init + (w_init - w_min) * np.exp(decay_rate * epoch)  
        return max(weight, w_min)
    '''
    def increase_weight(self, epoch, initial_weight, increase_rate=0.001):
        return initial_weight * increase_rate * epoch  # 0.001 - 
        #return initial_weight * np.exp(decay_rate * epoch)
    ...
    def compute_loss(self, batch, mode, epoch):
        mixed_loss = 0
        losses = {}
        for ltype, lam in self.lambdas.items():
            ...
            '''
            if ltype == 'velxyz':
                lam = self.decay_weight(epoch, lam, 0.001) 
            '''
            mixed_loss += loss*lam
    ...
    def generate_one(self, cls, duration, fact=1, xyz=False):
        ...
        output_xyz, _ = self.rot2xyz(batch["output"], batch["mask"])
