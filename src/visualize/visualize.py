...
from .anim import plot_3d_motion_dico, load_anim, plot_3d_motion
...
def generate_syn_by_video(generation, params, nats, nspa, tmp_path):
    # shape : (17, 3, 4, 480, 640, 3)
    # (nframes, row, column, h, w, 3)
    fps = params["fps"]
    ...
    nframes, num_joints, _ = generation['keypoint'][0].shape

    motions_array = np.array(generation['keypoint'])
    ...
    timesize = lenmax + 5
    import multiprocessing
    action_name = {0: 'Supine', 1: 'Prone', 2: 'Sitting', 3: 'Standing', 4: 'All-fours'}
    ...
    with multiprocessing.Pool() as pool:
        ...
        iterator = ((gener[i, j],
                     nframes,
                     save_path_format.format(i, j),
                     params, {"title": f"gen: {action_name[j]}", "interval": 1000/fps})
                    for j in range(nats) for i in range(nspa))
        gener["frames"] = pool_job_with_desc(pool, iterator,
                                             "{} the generated samples",
                                             nats*nspa,
...
def viz_epoch(model, dataset, epoch, params, folder, writer=None):
    ...
    with torch.no_grad():
        ...
        visualization["output_xyz"], visualization["output_pose"] = model.rot2xyz(visualization["output"], visualization["mask"])
...
def viz_generated_motions(generation, params, folder):
    """ Generate & viz samples """
    ...
    tmp_path = os.path.join(folder, f"subfigures_synthetic")
    os.makedirs(tmp_path, exist_ok=True)

    print("Generate the videos..")
    generate_syn_by_video(generation, params, nats, nspa, tmp_path)
...
def viz_dataset(dataset, params, folder):
    ...
    visualization["output_xyz"], visualization["output_pose"] = rot2xyz(output.to(device),
...
def viz_entire_dataset(dataset, params, folder):
    """ Generate & viz dataset """
    print("Visualization of the dataset")

    figname = "{}_{}".format(params["dataset"], params["pose_rep"])

    tmp_path = os.path.join(folder, f"subfigures_{figname}")
    os.makedirs(tmp_path, exist_ok=True)

    device = params["device"]    
    iterator = DataLoader(dataset, batch_size=params["batch_size"],
    ...
    for i, ori_batch in tqdm(enumerate(iterator), desc="Computing batch"):
        ...
        # Visualizaion of real samples
        visualization = {"x": ori_batch['x'],
                         "y": ori_batch['y'],
                         "mask": ori_batch['mask'],
                         "lengths": ori_batch['lengths'],
                         "output": ori_batch['x']}


        output = visualization["output"]
        visualization["output_xyz"], visualization["output_pose"] = rot2xyz(output.to(device), visualization["mask"].to(device), **rot2xyz_params)
        print(visualization["output_xyz"].size())

        generate_by_ori_video_sequences(i, visualization, params, tmp_path)


def generate_by_ori_video_sequences(iter_idx, visualization, params, tmp_path):
    # shape : (17, 3, 4, 480, 640, 3)
    # (nframes, row, column, h, w, 3)
    fps = params["fps"]
    ...
    for i in range(len(visu['lengths'])):
        plot_3d_motion(visu[outputkey][i], visu["lengths"][i], save_path_format.format(visu["y"][i], i + iter_idx * params["batch_size"]), params, title="", interval=50)
