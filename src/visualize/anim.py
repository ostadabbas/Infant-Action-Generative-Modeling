...
def plot_3d_motion(ori_motion, length, save_path, params, title="", interval=50):
    import matplotlib
    ...
    if torch.is_tensor(ori_motion):
        ori_motion = ori_motion.numpy()

    if ori_motion.shape[0] == 6890:
        model_path = './models/smpl/SMPL_NEUTRAL_inf.pkl'
        with open(model_path, 'rb') as f:
            smpl_params = pickle.load(f, encoding='latin1')

        J_regressor = smpl_params['J_regressor']
        print(J_regressor.shape) 

        motion= np.zeros((24, 3, 60))

        for x in range(ori_motion.shape[2]):

           current_frame_vertices = ori_motion[:, :, x]
    
           joints_positions = J_regressor.dot(current_frame_vertices)
    
           motion[:, :, x] = joints_positions
    else:
        motion = ori_motion
