...
class STGCN(nn.Module):
    ...
    def forward(self, batch):
        ...
        if batch['x'].size(1) - self.A.size(1) == 1:
            x = batch['x'][:, :-1, :, :]
        else:
            x = batch['x']
         
        x = x.permute(0, 2, 3, 1).unsqueeze(4).contiguous()
    ...
    def compute_accuracy(self, batch, mode):
